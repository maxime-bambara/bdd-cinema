<?php
//Config BDD WAMP
$mysqlDsn = 'mysql:dbname=cinema;host=localhost:8889';
$username = 'root';
$password = 'root';
...
CREATE TABLE films
(
  id_films INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE shows
(
  id_shows INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  cinema_seats TINYINT NULL,
  time_slot DATETIME NOT NULL,
  room_number TINYINT NOT NULL,
  select_film INT(11) NOT NULL,
  select_user INT(11) NOT NULL,
  FOREIGN KEY (select_film) REFERENCES films(id_films),
  FOREIGN KEY (select_user) REFERENCES users(id_users)
) ENGINE=InnoDB;
...
try{$obj_pdo = new PDO($mysqlDsn,$username,$password);
    $statement = $obj_pdo->prepare('INSERT INTO users (email, password, first_name, last_name, role ,birthday) 
    VALUES (:email, :password, :first_name, :last_name, :role ,:birthday)');
...
try {
    $obj_pdo = new PDO($mysqlDsn, $username, $password);
    $statement = $obj_pdo->prepare("SELECT cinema_seats as seats, time_slot, room_number, 
    name as film_name, first_name, last_name, email 
    FROM shows
	INNER JOIN films ON shows.select_film = films.id_films
    INNER JOIN users ON shows.select_user = users.id_users
    WHERE id_shows like :id;");
    $statement->bindValue(':id', '1');
    if ($statement->execute()) {
        while ($shows = $statement->fetch(PDO::FETCH_OBJ)) {
            echo 'La reservation de ' . $shows->first_name . ' ' .$shows->last_name .
                '(' .$shows->email . ')' .  ' : ' . $shows->film_name . '<br>' .
                $shows->time_slot . '<br>Nombre de place : ' . $shows->seats .
                '<br>Salle n°' . $shows->room_number . '<br>';
        }
    } else {
        $errorInfo = $statement->errorInfo();
        echo 'SQLSTATE : ' . $errorInfo[0] . '<br>';
